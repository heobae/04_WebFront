...
// 변수에 대입한 값 변경하기
number1 = 300; // number1에 300 재대입 (기존 10값 덮어쓰기)
...
// 2) 괄호 내부에 , 를 작성해 각각의 값을 독립적으로 출력
// 형변환 없이 출력되어 문자열 따로, 다른 데이터 타입
// 따로 독립되어 출력됨
...
// 1. var (변수ㅡ 변수 선언 시 중복되는 명수명으로 선언 가능)
...
console.log("menu : " + menu);
// 변수명이중복되어 값이 덮어쓰기 되면
// 이전에 선언 해놓은 변ㅇ수를 쓸 수 없게 되는 분제가 발생함.
...
number3 = 500; // 기존 변수 number3에 새로운값 재할당 가능
...
console.log(foo1);  // 전역 변수이기 때문에
console.log(foo2);  // 전역 변수이기 때문에
...
console.log(test2);
// -> test2도 접근 가능/
// -> 왜? 함수레벨 스코프인 var는 일반블록은 무시하기 때문에

// 함수안에서는 어떻게 될까?
function example() { // 이렇게 함수 형태로 만들어진
  // 코드블록은 함수레벨 scope라고 함.
  var test3 = '함수레벨 var 테스트';
  console.log(test3); // 선언만 된 상태. 결과값은 아직 안 나옴.
}

// console.log(test3); // Uncaught ReferenceError: test3 is not defined
// -> var는 일반 블록레벨만 무시할 뿐 함수레벨 블록은 무시하지 못함
// 즉, 함수블록 안에서 선언된 var 변수는 함수 안에서만 사용 가능하다.

example(); // console.log(test3);를 함수블록 안에 넣고 example();를 밖으로
...
// 초기화도 전에 호출?
// 에러는 안 나고 undefined만 뜸 (값만 안 들어감)
// 호이스팅 되어 var a; 선언부는 최상단으로 올라감.
// 선언은 되어있으니 a에 값이 대입되진 않아서
// undefined 값 반환
...
let undef; // 변수 선언
console.log("unfef : ", undef, typeof undef); // 자료형 undefined
// undefined : 정의되지 않은 변수 / 값이 아직 대입되지 않았다.

// String(문자열) : "" 또는 '' 로 작성된 값
const userName = "홍길동";
console.log("name : ", userName, typeof userName); // 자료형 String

const phone = '01012341234';
console.log("phone : ", phone, typeof phone); // 자료형 String

const gender = 'M'; // 한글자만 작성해도 문자열 (String)
console.log("gender : ", gender, typeof gender); // 자료형 String
...
console.log("age : ", age, typeof age); // 자료형 number
console.log("height : ", height, typeof height);  // 자료형 number
console.log("eyesight : ", eyesight, typeof eyesight);  // 자료형 number

// boolean값 (논리값 true / flase)
const isTure = true;
const isFalse = false;

console.log("isTrue : ", isTure, typeof isTure);  // 자료형 boolean
console.log("isFalse : ", isFalse, typeof isFalse); // 자료형 boolean
...
console.log("numbers: ", numbers, typeof numbers); // 자료형 number

console.log("numbers 배열값 중 0번째: ", numbers[0], typeof numbers[0]); // 자료형 number
console.log("numbers 배열값 중 1번째: ", numbers[1], typeof numbers[1]);
console.log("numbers 배열값 중 2번째: ", numbers[2], typeof numbers[2]);
console.log("numbers 배열값 중 3번째: ", numbers[3], typeof numbers[3]); // undefined 자료형 undefined
...
// 객체에 존재하는 값 하나씩 얻어오기
// 방법 1: 변수명['key']
console.log(user['id']); // 홑, 쌍따옴표 필요 o

// 방법 2: 변수명.key
console.log(user.id); // 홑, 쌍따옴표 필요 X
...
console.log(typeof sumFn); // 자료형 function

console.log(sumFn(3,5));

// null
console.log(typeof null); // 자료형 object